...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    y = blockIdx.x * bl + threadIdx.x;
    x = blockIdx.y * num_size + threadIdx.y;
...
int main(int argc, char** argv) {
    ...
    cudaMalloc((void **)&device_s, channels * height * width * sizeof(unsigned char));
    cudaMalloc((void **)&device_t, channels * height * width *  sizeof(unsigned char));
    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(device_s, host_s, channels * height * width * sizeof(unsigned char), cudaMemcpyHostToDevice); // dst , host , size 

    /* Hint 3 */
    // acclerate this function
    dim3 num_thread(num_size, num_size);
    dim3 num_block(height/num_size, width/num_size);
    sobel<<<num_block, num_thread>>>(device_s, device_t, height, width, channels);
    
    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, device_t, channels * height * width * sizeof(unsigned char), cudaMemcpyDeviceToHost);
