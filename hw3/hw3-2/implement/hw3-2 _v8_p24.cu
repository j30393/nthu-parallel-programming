...
__device__ void autogenerate(int* Dist_d , int* target , int x , int y , int x_d  , int y_d , int new_n){
    ...
    target[y * BlockSize + x + 2] = Dist_d[y_d * new_n + x_d + 2];
    target[y * BlockSize + x + 3 ] = Dist_d[y_d * new_n + x_d + 3];
}
...
__device__ void write_back(int* Dist_d , int* target , int x , int y , int x_d  , int y_d , int new_n){
    ...
    Dist_d[y_d * new_n + x_d + 2] = target[y * BlockSize + x + 2];
    Dist_d[y_d * new_n + x_d + 3] = target[y * BlockSize + x + 3];
}
...
__global__ void phase1(int* Dist_d, int round_cnt, int new_n){
    ...
    for(int i = 0 ; i < BlockSize ; i++){
        ...
        store[y * BlockSize + x + 2] = min(store[y * BlockSize + i ] + store[i* BlockSize + x + 2] , store[y * BlockSize + x + 2]);
        store[y * BlockSize + x + 3] = min(store[y * BlockSize + i ] + store[i* BlockSize + x + 3] , store[y * BlockSize + x + 3]);
        __syncthreads();
    }
...
__global__ void phase2(int* Dist_d, int round_cnt, int new_n){
    ...
    for(int i = 0 ; i < BlockSize ; i++){
        ...
        vertical[y * BlockSize + x + 2] = min( vertical[y * BlockSize + i] + store[i * BlockSize + x + 2] , vertical[y * BlockSize + x + 2]);
        vertical[y * BlockSize + x + 3] = min( vertical[y * BlockSize + i] + store[i * BlockSize + x + 3] , vertical[y * BlockSize + x + 3]);
        ...
        herizonal[y * BlockSize + x + 2] =  min( store[y * BlockSize + i] + herizonal[i * BlockSize + x + 2] , herizonal[y * BlockSize + x + 2]);
        herizonal[y * BlockSize + x + 3] =  min( store[y * BlockSize + i] + herizonal[i * BlockSize + x + 3] , herizonal[y * BlockSize + x + 3]);
    }

    write_back(Dist_d ,store , x , y , x_ver , y_her , new_n);
...
__global__ void phase3(int* Dist_d, int round_cnt, int new_n){
    ...
    for(int i = 0 ; i < BlockSize ; i++){
        ...
        store[y * BlockSize + x + 2] = min(vertical[y * BlockSize + i] + herizonal[i * BlockSize + x + 2] , store[y * BlockSize + x + 2]);
        store[y * BlockSize + x + 3] = min(vertical[y * BlockSize + i] + herizonal[i * BlockSize + x + 3] , store[y * BlockSize + x + 3]);
    }
