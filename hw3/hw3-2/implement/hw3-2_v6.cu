...
__device__ void autogenerate(int* Dist_d , int* target , int x , int y , int x_d  , int y_d , int new_n){
    ...
    target[y * BlockSize + (x + Half) ] = Dist_d[y_d * new_n + (x_d + Half)];
    target[(y + Half) * BlockSize + x ] = Dist_d[(y_d + Half) * new_n + x_d ];
    target[(y + Half) * BlockSize + (x + Half) ] = Dist_d[(y_d + Half) * new_n + (x_d + Half)];
}
...
__device__ void write_back(int* Dist_d , int* target , int x , int y , int x_d  , int y_d , int new_n){
    ...
    Dist_d[y_d * new_n + (x_d + Half)] = target[y * BlockSize + (x + Half) ];
    Dist_d[(y_d + Half) * new_n + x_d ] = target[(y + Half) * BlockSize + x ];
    Dist_d[(y_d + Half) * new_n + (x_d + Half)] = target[(y + Half) * BlockSize + (x + Half) ];
}
...
__global__ void phase1(int* Dist_d, int round_cnt, int new_n){
    ...
    int y = threadIdx.x;
    int x = threadIdx.y;
    int y_real = threadIdx.x + round_cnt * BlockSize;
    int x_real = threadIdx.y + round_cnt * BlockSize;
