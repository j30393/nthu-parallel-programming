...
__global__ void phase3(int* Dist_d, int round_cnt, int new_n , int offset ){
    __shared__ int store[BlockSize * BlockSize];
    __shared__ int vertical[BlockSize * BlockSize];
    __shared__ int herizonal[BlockSize * BlockSize];
    int x = threadIdx.x * 2;
    ...
    int y_ver =  (blockIdx.y + offset) * BlockSize + y;
...
int main(int argc, char* argv[]) {
    ...
    #pragma omp parallel
    {
        ...
        cudaMemcpy(deviceDist[cpu_thread_id], Dist, size_, cudaMemcpyHostToDevice);
